...
#sounds settings
sekme= "sounds/sekme.wav"
yemarti = "sounds/yemarti.wav"
yemeksi = "sounds/yemeksi.wav"
buffyem = "sounds/buffyem.wav"
debuffyem= "sounds/debuffyem.wav"
pikseleksi = "sounds/pikseleksi.wav"
pikselarti = "sounds/pikselarti.wav"
bitis = "sounds/bitis.wav"
hızlanma= "sounds/hızlanma.wav"
yavaslama= "sounds/yavaslama.wav"
...
while (secenek==1):
    ...
    if uzaklık3<20:
            ...
            if(b<=1):
                speedp1+=0.50
                system.sound(hızlanma) 
            if(b>=5):
                speedp1-=0.30
                system.sound(yavaslama) 
            if(b>1 and b<=3):
                score_player1+=2
                system.sound(buffyem) 
            if(b>3 and b<5):
                score_player1-=2
                system.sound(debuffyem) 
            scoreboardp1.clear()
            scoreboardp1.write("Player 1\nScore: {} \nSpeed: {}".format(score_player1,speedp1),font=FONT)
            buff.setposition(random.randint(-345,345),random.randint(-345,345))
...
while (secenek==2):  
    ...
    if uzaklık3<20:
            ...
            if(b>1 and b<=3):
                if(boyut1<5):
                    boyut1+=0.50
                    system.sound(pikselarti)
                    player1.turtlesize(boyut1)
            if(b>3 and b<5):
                if(boyut1>0.50):
                    boyut1-=0.25
                    system.sound(pikseleksi)
                    player1.turtlesize(boyut1)
